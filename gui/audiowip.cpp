...
int main(void)
{
    ...
    plm_t *plm = plm_create_with_filename("rick.mpeg");

    if (!plm) {
        TraceLog(LOG_ERROR, "Failed to open video file");
        return 1;
    }
    ...
    //     // Init raw audio stream (sample rate: 44100, sample size: 32bit,
    //     // channels: 2-stereo) WARNING: InitAudioDevice() inits internal double
    //     // buffering system to AUDIO_BUFFER_SIZE*2, but every audio sample is
    //     // PLM_AUDIO_SAMPLES_PER_FRAME, usually 1152 samples... Two solutions:
    //     // 1. Just change raudio internal AUDIO_BUFFER_SIZE to match
    //     // PLM_AUDIO_SAMPLES_PER_FRAME (1152)
    //     // 2. Keep internal raudio AUDIO_BUFFER_SIZE (4096) and fill it with
    //     // multiple plm audio samples,
    //     //    main issue is that (4096/1152 = 3.555) no round numbers, so, some
    //     //    samples should be divided into the double buffering system... not
    //     //    trivial to do...
    ...
    //         plm, (double) PLM_AUDIO_SAMPLES_PER_FRAME / (double) samplerate);
    // }
    ...
    // SetTargetFPS(100);              // Set our game to run at 100
    // frames-per-second
    //----------------------------------------------------------
    ...
    while (!WindowShouldClose()) // Detect window close button or ESC key
    {
        ...
        if (IsKeyPressed(KEY_R)) {
            ...
            plm_rewind(plm);
            plm_set_loop(plm, old);
        }
        if (IsKeyPressed(KEY_L)) {
            ended = false;
            plm_set_loop(plm, !plm->loop);
        }

        if (!pause && !ended) {
            // Video should run at 'framerate' fps => One new frame every
            // 1/framerate
        ...
        DrawTexture(
            texture, GetScreenWidth() / 2 - texture.width / 2, GetScreenHeight() / 2 - texture.height / 2,
            WHITE
        );
